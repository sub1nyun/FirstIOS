...
//Class는 참조 타입으로 init()으로 초기화를 필수로 해줘야함
//생성자를 만들어 줘야함 -> 메모리에 올림
//value 값과는 다르게 사본으로 상대방과 공유하는 것이 아닌
//위치를 알려주어 모든 곳에서 그 파일이 공유될 수 있도록 함

//단점으로 그 파일을 지우거나 수정하게 된다면 모든 곳에서 영향을 받음
//스위프트에서는 Class보다는 Struct를 사용할 것을 권장함

//#Class 단일 상속만 가능
class testName { // : 을 사용하여 상속을 받을 수 있음
    var name : String = "포뇨"
    var isfavorite : Bool = false
    
}

let mytestName : testName = testName()
myCharacter.name = "소피아"
myCharacter.isfavorie = false

//인스턴스 생성되고 -> 이니셜라이즈된 후에 var 이건 let 이건 내부 프로퍼티 값을 변경할 수 있음

//값 타입(구조체) : 값 타입 데이터 -> 함수의 전달인자로 전달(복사) -. 인스턴스 생성 -> 복사되어 들어감
